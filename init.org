...
#+BEGIN_SRC emacs-lisp
  ...
  ; It turns out its a lot easier just to use customize to handle themes and just let the user (me I guess) install and apply whatever themes they want.  
...
#+RESULTS:
| #[nil ((load-theme 'catppuccin)) nil] | org-persist-load-all | x-wm-set-size-hint | tramp-register-archive-autoload-file-name-handler | magit-maybe-define-global-key-bindings |
